...
class _DeviceScreenState extends State<DeviceScreen> {
  ...
  Future onScanWiFiPressed() async {
    if (mounted) {
      setState(() {
        _isDiscoveringServices = true;
      });
    ...
    try {
      ...
      Snackbar.show(ABC.c, "Scan WiFi: Success", success: true);
    } catch (e) {
      Snackbar.show(ABC.c, prettyException("Scan WiFi Error: ", e), success: false);
    }
    if (mounted) {
      setState(() {
        _isDiscoveringServices = false;
  ...
  List<Widget> _buildSsidTiles(BuildContext context, BluetoothDevice d) {
    return _ssids
        .map(
          (s) => SsidTile(
            ssid: s.$1,
  ...
  Widget buildScanWiFi(BuildContext context) {
    return IndexedStack(
      index: (_isDiscoveringServices) ? 1 : 0,
      children: <Widget>[
        TextButton(
          onPressed: onScanWiFiPressed,
